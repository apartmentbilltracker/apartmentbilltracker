...
const ClientTabNavigator = () => {
  ...
  const CustomTabBar = ({ state, descriptors, navigation }) => {
    ...
    const FAB_SIZE = 64;
    const FAB_LIFT = 34; // rises 34px above bar top — matches the tight notch cradle geometry
        ...
        <TouchableOpacity
          ...
          style={[
            styles.fabBtn,
            {
              ...
              bottom:
                tabInsets.bottom + TAB_BAR_HEIGHT / 2 - FAB_SIZE / 2 + FAB_LIFT,
  ...
  // SVG notch background — tight shallow cradle that hugs the FAB circle
  const NotchBackground = ({
    ...
    // Design space: 380 wide × 70 tall (bar height only, not insets)
    // FAB: 60px dia, 5px white border → outer radius 35px. Lifted 34px above bar top.
    // FAB centre in bar coords ≈ y = -4 (just above bar top edge).
    // Notch cradle radius ≈ 39px (35 + 4px breathing gap).
    // Bowl depth y=28 in 70px space → just deep enough to hug the FAB bottom.
    // Notch x-opening ≈ 148–232 (84px wide) so shoulders flare gently.
    const x = (n) => (n / 380) * width;
    const y = (n) => (n / 70) * height;

    const d = [
      `M 0,0`,
      `L ${x(148)},0`,
      // Left shoulder: flat approach → smooth S-curve down into the bowl
      `C ${x(160)},0 ${x(165)},0 ${x(170)},${y(5)}`,
      `C ${x(175)},${y(10)} ${x(178)},${y(18)} ${x(182)},${y(23)}`,
      // Bowl bottom: shallow tight U — not a deep well
      `C ${x(185)},${y(27)} ${x(188)},${y(28)} ${x(190)},${y(28)}`,
      `C ${x(192)},${y(28)} ${x(195)},${y(27)} ${x(198)},${y(23)}`,
      // Right shoulder: mirror of left
      `C ${x(202)},${y(18)} ${x(205)},${y(10)} ${x(210)},${y(5)}`,
      `C ${x(215)},0 ${x(220)},0 ${x(232)},0`,
      `L ${width},0`,
      `L ${width},${height}`,
      `L 0,${height}`,
      `Z`,
    ...
    // Border stroke — same path offset 0.5px down
    const stroke = [
      `M 0,0.5 L ${x(148)},0.5`,
      `C ${x(160)},0.5 ${x(165)},0.5 ${x(170)},${y(5) + 0.5}`,
      `C ${x(175)},${y(10) + 0.5} ${x(178)},${y(18) + 0.5} ${x(182)},${y(23) + 0.5}`,
      `C ${x(185)},${y(27) + 0.5} ${x(188)},${y(28) + 0.5} ${x(190)},${y(28) + 0.5}`,
      `C ${x(192)},${y(28) + 0.5} ${x(195)},${y(27) + 0.5} ${x(198)},${y(23) + 0.5}`,
      `C ${x(202)},${y(18) + 0.5} ${x(205)},${y(10) + 0.5} ${x(210)},${y(5) + 0.5}`,
      `C ${x(215)},0.5 ${x(220)},0.5 ${x(232)},0.5`,
      `L ${width},0.5`,
    ].join(" ");
...
const styles = StyleSheet.create({
  ...
  fabBtn: {
    ...
    alignSelf: "center",
    left: "50%",
    marginLeft: -30, // half of FAB_SIZE
    alignItems: "center",
    justifyContent: "center",
    elevation: 8,
    shadowOffset: { width: 0, height: 4 },
