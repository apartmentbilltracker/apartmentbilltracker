...
const ClientTabNavigator = () => {
  ...
  const CustomTabBar = ({ state, descriptors, navigation }) => {
    ...
    const FAB_SIZE = 68;
    const FAB_LIFT = 38; // FAB rises 38px above bar — sits high in the wide shallow notch
  ...
  // very shallow bowl (y=32 in 72-space), smooth barely-visible transition from flat bar
  const NotchBackground = ({
    ...
    // Design space: 380 × 72
    // BPI characteristics:
    //   - Notch starts very early (x=120) and ends very late (x=260) — wide opening
    //   - First control points far from centre keep the approach nearly invisible
    //   - Only 2 cubic segments per side (not 4) — simpler, smoother
    //   - Bowl bottom at y=32 (shallow — FAB sits mostly above the bar)
    //   - Bowl is very narrow at the bottom (x=186 to x=194) — tight V-bottom with rounded tip
    const x = (n) => (n / 380) * width;
    const y = (n) => (n / 80) * height;

    const d = [
      `M 0,0`,
      `L ${x(142)},0`,
      // Left shoulder: long flat approach, single smooth sweep down
      `C ${x(132)},0 ${x(144)},0 ${x(149)},${y(8)}`,
      `C ${x(150)},${y(12)} ${x(154)},${y(25)} ${x(159)},${y(30)}`,
      // Bowl: narrow bottom, rounded tip
      `C ${x(165)},${y(38)} ${x(170)},${y(42)} ${x(180)},${y(44)}`,
      `C ${x(210)},${y(48)} ${x(214)},${y(35)} ${x(220)},${y(32)}`,
      // Right shoulder: mirror
      `C ${x(222)},${y(30)} ${x(234)},${y(12)} ${x(232)},${y(8)}`,
      `C ${x(240)},0 ${x(250)},0 ${x(260)},0`,
      `L ${width},0`,
      `L ${width},${height}`,
      `L 0,${height}`,
      `Z`,
    ...
    const stroke = [
      `M 0,0.5`,
      `L ${x(120)},0.5`,
      `C ${x(145)},0.5 ${x(158)},0.5 ${x(168)},${y(4) + 0.5}`,
      `C ${x(178)},${y(8) + 0.5} ${x(182)},${y(18) + 0.5} ${x(186)},${y(26) + 0.5}`,
      `C ${x(188)},${y(30) + 0.5} ${x(189)},${y(32) + 0.5} ${x(190)},${y(32) + 0.5}`,
      `C ${x(191)},${y(32) + 0.5} ${x(192)},${y(30) + 0.5} ${x(194)},${y(26) + 0.5}`,
      `C ${x(198)},${y(18) + 0.5} ${x(202)},${y(8) + 0.5} ${x(212)},${y(4) + 0.5}`,
      `C ${x(222)},0.5 ${x(235)},0.5 ${x(260)},0.5`,
      `L ${width},0.5`,
    ].join(" ");
...
const styles = StyleSheet.create({
  ...
  fabBtn: {
    ...
    alignSelf: "center",
    left: "50%",
    marginLeft: -34, // half of FAB_SIZE
    alignItems: "center",
    justifyContent: "center",
    elevation: 8,
    shadowOffset: { width: 0, height: 4 },
